...
// Katalog 100 mode mengikuti firmware baru: 5 mesin animasi x 20 palet.
// ID tetap wled001 ... wled100 agar pairing, favorit, dan API lama tetap kompatibel.
const List<String> _wledEngines = [
  "Static", "Rainbow Run", "Rainbow Cycle", "Disco", "Confetti",
];
const List<String> _wledPalettes = [
  "Rainbow", "Fire", "Ice", "Ocean", "Forest",
  "Sunset", "Party", "Red", "Green", "Blue",
  "Purple", "Warm White", "Cool White", "Pastel", "Red-Blue",
  "Pink-Cyan", "Gold", "Neon", "Candy", "Mono",
];

List<String> get _wledEffectNames => [
      for (final engine in _wledEngines)
        for (final palette in _wledPalettes) "$engine - $palette",
    ];

List<Map<String, String>> get _wledEffects => List.generate(
      _wledEffectNames.length,
      (index) => {
        "id": "wled${(index + 1).toString().padLeft(3, "0")}",
        "name": _wledEffectNames[index],
      },
    );

// 20 pola animasi mode Custom: solid (warna rata) + 19 animasi bergerak.
// id di sini harus SAMA PERSIS dengan yang dikenali firmware.ino (lihat
// isValidCustomPattern() di firmware).
const List<(String id, String label, IconData icon)> _customPatterns = [
  ("solid", "Solid", Icons.circle_rounded),
  ("chaseRight", "Jalan Kanan", Icons.arrow_forward_rounded),
  ("chaseLeft", "Jalan Kiri", Icons.arrow_back_rounded),
  ("pingpong", "Ping-Pong", Icons.swap_horiz_rounded),
  ("pulse", "Berdenyut", Icons.favorite_rounded),
  ("breathe", "Bernapas", Icons.air_rounded),
  ("rainbow", "Rainbow", Icons.gradient_rounded),
  ("rainbowRight", "Rainbow Kanan", Icons.arrow_circle_right_rounded),
  ("rainbowLeft", "Rainbow Kiri", Icons.arrow_circle_left_rounded),
  ("spinCW", "Memutar CW", Icons.rotate_right_rounded),
  ("spinCCW", "Memutar CCW", Icons.rotate_left_rounded),
  ("theaterChase", "Theater Chase", Icons.view_column_rounded),
  ("cometRight", "Comet Kanan", Icons.east_rounded),
  ("cometLeft", "Comet Kiri", Icons.west_rounded),
  ("sparkle", "Sparkle", Icons.auto_awesome_rounded),
  ("twinkle", "Twinkle", Icons.star_border_rounded),
  ("wipeRight", "Color Wipe Kanan", Icons.line_weight_rounded),
  ("wipeLeft", "Color Wipe Kiri", Icons.line_weight_rounded),
  ("centerOut", "Center Out", Icons.expand_rounded),
  ("edgeIn", "Edge In", Icons.compress_rounded),
];
...
// =======================================================================
// ===== SPLASH SCREEN — VP ENERGY REVEAL ================================
// =======================================================================
...
/// Video referensi dipakai langsung agar bentuk petir, timing, glow, ledakan,
/// ring, partikel, dan loading benar-benar sama dengan animasi yang dikirimkan.
class _SplashScreenState extends State<SplashScreen> {
  ...
  void initState() {
    super.initState();
    _video = VideoPlayerController.asset('assets/splash/vp_splash.mp4');
    _video.addListener(_watchVideo);
    _video.initialize().then((_) {
      if (!mounted) return;
      setState(() => _ready = true);
      _video.play();
    }).catchError((Object error) {
      // Jika decoder video gagal pada perangkat tertentu, lanjutkan ke menu
      // agar aplikasi tetap dapat digunakan.
      if (mounted) _goToApp();
    });
  }

  void _watchVideo() {
    if (!_video.value.isInitialized || _didNavigate) return;
    final duration = _video.value.duration;
    final position = _video.value.position;
    if (duration > Duration.zero && position >= duration - const Duration(milliseconds: 120)) {
      _goToApp();
    }
  }
...
class _LightningPainter extends CustomPainter {
  ...
  _LightningPainter({required this.progress, required this.impact, required this.explosion,
    required this.loop, required this.color, required this.blue});

  Offset _lerp(Offset a, Offset b, double t) => Offset(
    a.dx + (b.dx - a.dx) * t, a.dy + (b.dy - a.dy) * t,
  );

  List<Offset> _boltPoints(Offset start, Offset target, double seed) {
    final points = <Offset>[start];
    for (int i = 1; i <= 15; i++) {
      final p = i / 15;
      final wobble = sin(seed * 8.0 + i * 2.2) * 24 * (1 - p) + cos(seed + i * 1.6) * 8;
      points.add(Offset(start.dx + (target.dx - start.dx) * p + wobble,
        start.dy + (target.dy - start.dy) * p));
    }
    return points;
  }

  void _drawBolt(Canvas canvas, List<Offset> points, double amount, double seed) {
    if (amount <= 0) return;
    final count = points.length - 1;
    final exact = amount * count;
    final visible = exact.ceil().clamp(1, count).toInt();
    final path = Path()..moveTo(points.first.dx, points.first.dy);
    for (int i = 1; i < visible; i++) path.lineTo(points[i].dx, points[i].dy);
    path.lineTo(_lerp(points[visible - 1], points[visible], exact % 1).dx,
      _lerp(points[visible - 1], points[visible], exact % 1).dy);
    final flicker = .72 + .28 * sin(seed * 17 + loop * 16 * pi).abs();

    // Beberapa lapisan offset membuat volume 3D: aura, depth biru, tubuh warna, inti putih.
    final glow = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round..strokeWidth = 19..color = blue.withOpacity(.18 * flicker);
    final depth = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round..strokeWidth = 8..color = const Color(0xFF071B4D).withOpacity(.92 * flicker);
    final body = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round..strokeWidth = 4..color = color.withOpacity(.98 * flicker);
    final core = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round..strokeWidth = 1.25..color = Colors.white.withOpacity(.94 * flicker);
    canvas.drawPath(path, glow);
    canvas.save(); canvas.translate(3.5, 6); canvas.drawPath(path, depth); canvas.restore();
    canvas.drawPath(path, body); canvas.drawPath(path, core);
  }

  @override
  void paint(Canvas canvas, Size size) {
    final target = Offset(size.width * .5, size.height * .405);
    final starts = <Offset>[
      Offset(size.width * 1.05, -26), Offset(-26, -26),
      Offset(size.width * 1.05, size.height + 26), Offset(-26, size.height + 26),
    ];
    for (int i = 0; i < starts.length; i++) {
      final delay = i * .10;
      final local = ((progress - delay) / (.78 - delay)).clamp(0.0, 1.0).toDouble();
      _drawBolt(canvas, _boltPoints(starts[i], target, i * 1.61 + .4), local, i * 1.61 + .4);
    }

    if (impact > 0) {
      final flash = Paint()..maskFilter = const MaskFilter.blur(BlurStyle.normal, 42)
        ..color = color.withOpacity(.16 * impact);
      canvas.drawCircle(target, 34 + impact * 48, flash);
    }
    if (explosion > 0) {
      final pulse = Paint()..style = PaintingStyle.stroke..strokeWidth = 4
        ..color = Colors.white.withOpacity((1 - explosion) * .82);
      canvas.drawCircle(target, 20 + explosion * 172, pulse);
      final spark = Paint()..style = PaintingStyle.stroke..strokeCap = StrokeCap.round
        ..strokeWidth = 2.8..color = color.withOpacity((1 - explosion) * .92);
      for (int i = 0; i < 28; i++) {
        final angle = i * pi * 2 / 28 + loop * pi * 2;
        final inner = 22 + (i % 4) * 5;
        final outer = inner + explosion * (46 + (i % 5) * 12);
        canvas.drawLine(
          Offset(target.dx + cos(angle) * inner, target.dy + sin(angle) * inner),
          Offset(target.dx + cos(angle) * outer, target.dy + sin(angle) * outer), spark);
      }
    }
  }
...
class _VpMarkPainter extends CustomPainter {
  final double progress;
  final Color color;

  _VpMarkPainter({required this.progress, required this.color});
  ...
  Path _pPath() => Path()
    ..moveTo(62, 87)
    ..lineTo(62, 17)
    ..cubicTo(91, 8, 105, 17, 104, 31)
    ..cubicTo(103, 46, 84, 48, 62, 43);

  Path _partial(Path path, double amount) {
    final metrics = path.computeMetrics().toList();
    final result = Path();
    var remaining = amount;
    for (final metric in metrics) {
      if (remaining <= 0) break;
      final length = min(metric.length, remaining);
  ...
  @override
  void paint(Canvas canvas, Size size) {
    final scale = min(size.width / 116, size.height / 104);
    canvas.save();
    canvas.translate((size.width - (116 * scale)) / 2, (size.height - (104 * scale)) / 2);
    canvas.scale(scale);

    final vProgress = progress.clamp(0.0, 1.0).toDouble();
    final pProgress = ((progress - .28) / .72).clamp(0.0, 1.0).toDouble();
    final v = _partial(_vPath(), vProgress * 115);
    final p = _partial(_pPath(), pProgress * 190);
    final depthPaint = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 10
      ..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round
      ..color = const Color(0xFF02101D).withOpacity(.8);
    final frontPaint = Paint()
      ...
      ..shader = LinearGradient(
        colors: [Colors.white, color, const Color(0xFF8AEFFF)],
        begin: Alignment.topLeft,
        end: Alignment.bottomRight,
      ).createShader(const Rect.fromLTWH(0, 0, 116, 104));

    for (int i = 5; i >= 1; i--) {
      canvas.save();
      canvas.translate(i * 1.4, i * 1.8);
      canvas.drawPath(v, depthPaint);
      canvas.drawPath(p, depthPaint);
      canvas.restore();
    }
    canvas.drawPath(v, frontPaint);
    canvas.drawPath(p, frontPaint);
    canvas.restore();
  }

  @override
  bool shouldRepaint(covariant _VpMarkPainter oldDelegate) =>
...
class _CircuitPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1
      ..color = const Color(0xFF35D9FF).withOpacity(.12);
    final nodePaint = Paint()..color = const Color(0xFF61EFFF).withOpacity(.28);

    for (int lane = 0; lane < 13; lane++) {
      final y = size.height * (0.04 + lane * .077);
      final startX = (lane * 83.0) % size.width;
      final path = Path()..moveTo(startX, y);
      var x = startX;
      var currentY = y;
      for (int segment = 0; segment < 4; segment++) {
        x += 32 + ((lane * 17 + segment * 23) % 90);
        currentY += (segment.isEven ? 1 : -1) * (10 + ((lane + segment) % 3) * 7);
        path.lineTo(x, currentY);
        path.lineTo(x + 26, currentY);
      }
      canvas.drawPath(path, paint);
      canvas.drawCircle(Offset(x, currentY), 2.1, nodePaint);
    }

    for (int i = 0; i < 12; i++) {
      final x = size.width * (0.05 + i * .085);
      final y = size.height * (0.18 + ((i * 37) % 62) / 100);
      canvas.drawCircle(Offset(x, y), 1.4 + (i % 3), nodePaint);
    }
  }
...
class _ParticlePainter extends CustomPainter {
  ...
  // Paint di-cache sekali (bukan dibuat ulang tiap frame) supaya tidak ada
  // alokasi objek baru 60x/detik - mengurangi beban GC, bikin frame pacing
  // lebih stabil/smooth terutama di HP kelas bawah.
  final Paint _paint = Paint();

  @override
  void paint(Canvas canvas, Size size) {
    for (final p in particles) {
      // BUG LAMA: p.speed sudah dihitung random tapi tidak pernah dipakai,
      // jadi semua partikel bergerak di kecepatan sama (cuma beda fase) -
      // hasilnya terasa mekanis/seragam. Sekarang speed benar-benar
      // memengaruhi laju naik tiap partikel, jadi terasa lebih organik.
      final progress = (loop * p.speed + p.phase) % 1.0;
      final y = size.height * (1 - progress);
      final x = p.x * size.width + sin((progress + p.phase) * 2 * pi) * p.drift;
      final opacity = sin(progress * pi).clamp(0.0, 1.0);
      _paint.color = color.withOpacity(0.55 * opacity);
...
class _ControllerPageState extends State<ControllerPage> with SingleTickerProviderStateMixin {
  late final AnimationController _menuEntranceCtrl;
  ...
  BluetoothDevice? _bleConnectingDevice;
  int _bleOperationId = 0;
  Timer? _bleChunkResetTimer;
  ...
  int fanRpm = 0; // RPM aktual fan, dari field "fanRpm" firmware (hasil baca tachometer)  bool peltierOn = false; // ON/OFF peltier (MOSFET low-side), dari field "peltier" firmware
  String ledMode = "off"; // built-in modes plus wled001 ... wled100
  int ledSpeed = 50; // kecepatan animasi LED (1-100), dari field "ledSpeed" firmware
  ...
  String customPattern = "solid"; // pola animasi mode Custom: solid/chaseRight/chaseLeft/pulse/spin/sparkle
  Color wledColor = Colors.white; // warna KHUSUS effect wled yang lagi aktif, dari field "wledColor" firmware
  String lastLedEffect = "running"; // efek terakhir dipilih, dipakai saat tombol ON
  final Set<String> favoriteLedModes = <String>{};
  bool ledMenuExpanded = false;
  bool ledCustomPanelExpanded = false;
  String? _pendingLedMode;
  DateTime? _pendingLedModeUntil;
  bool _ledOffIntent = false;
  DateTime? _lastLedReassertAt;
  ...
  // Penanda "lagi digeser jari" per slider - selama true, update status yang
  // masuk dari device (BLE notify tiap ~300ms / polling WiFi) TIDAK BOLEH
  // menimpa nilai slider ini. Tanpa ini, nilai lama dari device terus
  // "menang" di tengah drag dan bikin thumb-nya mental-mental/loncat-loncat
  // ngelawan jari - device baru dapat nilai final pas onChangeEnd (jari
  // dilepas), padahal status lama masih dikirim terus selama itu.
  bool _fanSliderDragging = false;
  bool _ledSpeedSliderDragging = false;
  bool _ledBrightnessSliderDragging = false;
  bool _oledTextSpeedSliderDragging = false;
  ...
  void initState() {
    super.initState();
    _menuEntranceCtrl = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 2300),
    );
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) _menuEntranceCtrl.forward();
    });
    _initApp();
  }
  ...
  void dispose() {
    ...
    _bleChunkResetTimer?.cancel();
    _oledTextController.dispose();
    _oledTextFocus.dispose();
    _menuEntranceCtrl.dispose();
    super.dispose();
  }

  Widget _menuEntry(int index, Widget child) => _StaggeredMenuEntry(
        controller: _menuEntranceCtrl,
        index: index,
        child: child,
      );
  ...
  Future<void> _loadFavoriteLedModes() async {
    final prefs = await SharedPreferences.getInstance();
    final saved = prefs.getStringList("favoriteLedModes") ?? <String>[];
    if (!mounted) return;
    setState(() {
      favoriteLedModes
        ..clear()
        ..addAll(saved.where((id) => _wledEffects.any((effect) => effect["id"] == id)));
    });
  }

  Future<void> _toggleFavoriteLedMode(String id) async {
    setState(() {
      if (!favoriteLedModes.add(id)) favoriteLedModes.remove(id);
    });
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList("favoriteLedModes", favoriteLedModes.toList());
  }
  ...
  Future<void> _connectActiveCooler() async {
    if (activeCooler == null) return;
    if (activeCooler!.mode == "WiFi") {
      await connectLocalWifi();
    } else {
      await _connectBleById(activeCooler!.bleRemoteId);
    }
  }
  ...
  Future<void> _connectBleById(String? remoteId, {int maxAttempts = 5}) async {
    ...
    final operationId = ++_bleOperationId;
    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
      if (operationId != _bleOperationId) return;
      try {
        final device = BluetoothDevice.fromId(remoteId);
        await connectBLE(device);
        if (operationId != _bleOperationId) return;
        if (bleConnected && bleDevice?.remoteId.str == remoteId) return;
      } catch (e) {}
      if (attempt < maxAttempts) {
        await Future.delayed(const Duration(seconds: 2));
      }
    }
    if (operationId == _bleOperationId && !bleConnected) {
  ...
  Future<void> switchActiveCooler(Cooler cooler) async {
    // Batalkan retry/koneksi lama dan tunggu disconnect selesai sebelum
    // menyentuh cooler baru. Kalau tidak, Android sering menolak koneksi
    // kedua sampai aplikasi direstart.
    ...
    await _connectActiveCooler();
  }

  Future<void> removeCooler(Cooler cooler) async {
    final removingActive = activeCooler?.id == cooler.id;
    if (removingActive) {
      await _disconnectBle();
      _stopLocalWifi();
    }
    ...
    await _savePairedCoolers();
    if (activeCooler != null) await _connectActiveCooler();
  }

  Future<void> addCooler(Cooler cooler, {BluetoothDevice? bleDeviceToUse}) async {
    await _disconnectBle();
    _stopLocalWifi();
    setState(() {
    ...
    await _savePairedCoolers();
    // Saat memilih hasil scan, gunakan objek device yang sudah ditemukan.
    // Jangan memulai koneksi kedua lewat _connectActiveCooler().
    if (bleDeviceToUse != null) {
      ++_bleOperationId;
      await connectBLE(bleDeviceToUse);
    } else {
      await _connectActiveCooler();
    }
  }
  ...
  Future<void> _disconnectBle() async {
    ++_bleOperationId;
    final device = bleDevice ?? _bleConnectingDevice;
    _bleConnectingDevice = null;
    await _bleNotifySubscription?.cancel();
    _bleNotifySubscription = null;
    _controlChar = null;
    _resetBleChunkBuffer();
    if (mounted) {
      setState(() {
        bleConnected = false;
        bleDevice = null;
  ...
  void _applyDeviceStatus(Map<String, dynamic> data) {
    ...
    if (rawFanSpeed is num && !_fanSliderDragging) fanSpeed = rawFanSpeed.toInt();
    if (rawFanRpm is num) fanRpm = rawFanRpm.toInt();
    if (rawPeltier is bool) peltierOn = rawPeltier;
    if (rawLedSpeed is num && !_ledSpeedSliderDragging) ledSpeed = rawLedSpeed.toInt();
    if (rawLedBrightness is num && !_ledBrightnessSliderDragging) ledBrightness = rawLedBrightness.toInt();
    ...
    final rawCustomPattern = data['customPattern'];
    if (rawCustomPattern is String) customPattern = rawCustomPattern;
    final rawWledColor = data['wledColor'];
    if (rawWledColor is String && rawWledColor.length == 6) {
      final parsed = int.tryParse(rawWledColor, radix: 16);
      if (parsed != null) wledColor = Color(0xFF000000 | parsed);
    }
    final rawLedMode = data['ledMode'];
    if (rawLedMode is String) {
      final pendingMode = _pendingLedMode;
      final pendingUntil = _pendingLedModeUntil;
      final pendingIsActive = pendingMode != null &&
          pendingUntil != null &&
          DateTime.now().isBefore(pendingUntil);
      final now = DateTime.now();
      final staleStatus = pendingIsActive &&
          pendingMode != rawLedMode &&
          ((rawLedMode == "off" && pendingMode != "off") ||
              (pendingMode == "off" && rawLedMode != "off"));
      final unexpectedOff = rawLedMode == "off" && ledMode != "off" && !_ledOffIntent;

      // A status poll can arrive just after a command and still contain the
      // old "off" value. Do not let that stale packet undo the mode the user
      // just selected. A later status packet is accepted normally.
      if (staleStatus || unexpectedOff) {
        // Keep the optimistic selection. If the device really lost the mode,
        // re-send it once instead of silently switching the UI to OFF.
        if (unexpectedOff &&
            (_lastLedReassertAt == null || now.difference(_lastLedReassertAt!).inSeconds >= 8)) {
          _lastLedReassertAt = now;
          final desiredMode = ledMode;
          Future<void>.microtask(() => sendLed(desiredMode));
        }
      } else if (_ledOffIntent && rawLedMode != "off") {
        // A delayed non-off packet must not undo an explicit OFF action.
      } else {
        if (ledMode != rawLedMode) ledCustomPanelExpanded = false;
        ledMode = rawLedMode;
        if (pendingMode == rawLedMode) {
          _pendingLedMode = null;
          _pendingLedModeUntil = null;
        }
        if (rawLedMode != "off") _ledOffIntent = false;
      }
    }
    ...
    if (rawOledTextSpeed is num && !_oledTextSpeedSliderDragging) oledTextSpeed = rawOledTextSpeed.toInt();
  ...
  Future<void> connectBLE(BluetoothDevice device) async {
    final operationId = _bleOperationId;
    _bleConnectingDevice = device;
    try {
      await _bleNotifySubscription?.cancel();
      _bleNotifySubscription = null;
      _resetBleChunkBuffer();
      await device.connect();
      if (!mounted || operationId != _bleOperationId) {
        try {
          await device.disconnect();
        } catch (_) {}
        return;
      ...
      if (operationId != _bleOperationId) {
        try {
          await device.disconnect();
        } catch (_) {}
        return;
      ...
      for (var service in services) {
        for (var characteristic in service.characteristics) {
          if (characteristic.uuid.toString() == "beb5483e-36e1-4688-b7f5-ea07361b26a8") {
            ...
            _bleNotifySubscription = characteristic.onValueReceived.listen((value) {
              if (operationId != _bleOperationId) return;
      ...
      if (operationId != _bleOperationId || _controlChar == null) {
        await _disconnectBle();
        return;
      }
      // Minta snapshot terbaru segera setelah notification aktif. Status
      // berkala 300 ms tetap berjalan sebagai backup, tetapi UI tidak perlu
      // menunggu kebetulan timing paket berikutnya saat app dibuka ulang.
      await _requestBleStatus();
      ...
      if (operationId == _bleOperationId && mounted) {
        setState(() {
          bleConnected = false;
          bleDevice = null;
          _controlChar = null;
      ...
      if (operationId == _bleOperationId && activeCooler != null) {
        HistoryService.recordStatus(coolerId: activeCooler!.id, online: false, voltage: setVolt);
      }
      if (operationId == _bleOperationId) {
        try {
      ...
      if (_bleConnectingDevice?.remoteId == device.remoteId) {
        _bleConnectingDevice = null;
      }
    }
  }

  void _resetBleChunkBuffer() {
    _bleChunkResetTimer?.cancel();
    _bleChunkResetTimer = null;
    _bleChunkBuffer = [];
    _bleChunkExpectedTotal = 0;
    _bleChunkReceivedCount = 0;
  }

  void _armBleChunkResetTimer() {
    _bleChunkResetTimer?.cancel();
    _bleChunkResetTimer = Timer(const Duration(milliseconds: 700), _resetBleChunkBuffer);
  }

  Future<void> _requestBleStatus() async {
    if (!bleConnected || bleDevice == null || _controlChar == null) return;
    await _writeControlBLE({"requestStatus": true});
  }
  ...
  // Firmware meminta ATT MTU 517 dan app meminta 512, jadi payload aman
  // sampai sekitar 509 byte. 480 memberi ruang untuk header 2 byte dan
  // mengurangi jumlah paket perintah jadwal/konfigurasi.
  static const int _bleWriteChunkSize = 480;
  ...
  Future<bool> _writeControlBLE(Map<String, dynamic> payload) async {
    ...
    try {
      ...
      for (int i = 0; i < totalChunks; i++) {
        ...
        if (totalChunks > 1) await Future.delayed(const Duration(milliseconds: 55));
      }
      return true;
    } catch (e) {
      return false;
  ...
  void sendLed(String mode) {
    ...
    _ledOffIntent = mode == "off";
    _pendingLedMode = mode;
    _pendingLedModeUntil = DateTime.now().add(const Duration(seconds: 4));
    setState(() => ledMode = mode);
    if (connectionMode == "WiFi") {
  ...
  void _setLedPower(bool enabled) {
    final nextMode = enabled
        ? (lastLedEffect == "off" ? "running" : lastLedEffect)
        : "off";
    sendLed(nextMode);
  }
  ...
  // Kirim pola animasi mode Custom (solid/chaseRight/chaseLeft/pulse/spin/sparkle).
  void sendCustomPattern(String pattern) {
    if (activeCooler == null) {
      _showSnack("⚠️ Pilih atau tambah cooler dulu");
      return;
    }
    if (connectionMode == "WiFi") {
      sendCustomPatternLocalWifi(pattern);
    } else {
      sendCustomPatternBLE(pattern);
    }
    setState(() => customPattern = pattern);
  }

  void sendCustomPatternLocalWifi(String pattern) async {
    if (_wifiIp == null) {
      _showSnack("⚠️ Belum menemukan ESP32 di jaringan, tunggu sebentar / cek WiFi HP");
      return;
    }
    try {
      final response = await http
          .post(Uri.http(_wifiIp!, "/set", {"customPattern": pattern}), headers: esp32AuthHeaders(activeCooler))
          .timeout(Duration(seconds: 3));
      if (response.statusCode == 200) {
        try {
          final data = jsonDecode(response.body);
        ...
        _showSnack("❌ ESP32 menolak pola custom");
      }
    } catch (e) {
      _showSnack("⚠️ Gagal kirim perintah, cek koneksi WiFi");
    }
  ...
  void sendCustomPatternBLE(String pattern) async {
    if (!bleConnected || bleDevice == null) {
      setState(() => status = "🔴 Offline");
      _showSnack("⚠️ Belum terhubung ke perangkat Bluetooth");
      return;
    }
    final ok = await _writeControlBLE({"customPattern": pattern});
    if (!ok) {
      _showSnack("❌ Gagal mengirim perintah ke perangkat");
    }
  }

  // Kirim warna KHUSUS utk 1 effect wled yang lagi aktif (ledMode == wledNNN).
  // Tersimpan sendiri-sendiri per mode di firmware - gak numpuk sama mode wled lain.
  void sendWledColor(Color color) {
    if (activeCooler == null) {
      _showSnack("⚠️ Pilih atau tambah cooler dulu");
      return;
    }
    final hex = color.value.toRadixString(16).padLeft(8, '0').substring(2).toUpperCase();
    if (connectionMode == "WiFi") {
      sendWledColorLocalWifi(hex);
    } else {
      sendWledColorBLE(hex);
    }
    setState(() => wledColor = color);
  }

  void sendWledColorLocalWifi(String hex) async {
    if (_wifiIp == null) {
      _showSnack("⚠️ Belum menemukan ESP32 di jaringan, tunggu sebentar / cek WiFi HP");
      return;
    }
    try {
      final response = await http
          .post(Uri.http(_wifiIp!, "/set", {"wledColor": hex}), headers: esp32AuthHeaders(activeCooler))
          .timeout(Duration(seconds: 3));
      if (response.statusCode == 200) {
        try {
          final data = jsonDecode(response.body);
        ...
        _showSnack("❌ ESP32 menolak warna mode ini");
      }
    } catch (e) {
      _showSnack("⚠️ Gagal kirim perintah, cek koneksi WiFi");
    }
  ...
  void sendWledColorBLE(String hex) async {
    if (!bleConnected || bleDevice == null) {
      setState(() => status = "🔴 Offline");
      _showSnack("⚠️ Belum terhubung ke perangkat Bluetooth");
      return;
    }
    final ok = await _writeControlBLE({"wledColor": hex});
    if (!ok) {
      _showSnack("❌ Gagal mengirim perintah ke perangkat");
    }
  }
                      ...
                      ...scanResults.map((r) {
                        ...
                        return ListTile(
                          ...
                          onTap: () async {
                            ...
                            await addCooler(cooler, bleDeviceToUse: r.device);
                          },
                        );
                      }).toList(),
                      TextButton.icon(
      ...
      body: Stack(children: [
        ...
        SafeArea(child: RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              _menuEntry(0, _nexusTicker(isDark)),
              const SizedBox(height: 12),
              _menuEntry(1, _nexusDeviceCard(isDark, online)),
              const SizedBox(height: 12),
              _menuEntry(2, _nexusHero(isDark, online)),
              const SizedBox(height: 12),
              _menuEntry(3, _nexusVoltageGrid(isDark)),
              const SizedBox(height: 12),
              _menuEntry(8, _nexusFanControl(isDark)),
              const SizedBox(height: 12),
              _menuEntry(9, _nexusPeltierControl(isDark)),
              const SizedBox(height: 12),
              _menuEntry(10, _nexusMetrics(isDark)),
              const SizedBox(height: 12),
              _menuEntry(11, _nexusRgbCard(isDark)),
              const SizedBox(height: 12),
              _menuEntry(12, _nexusOledCard(isDark)),
              const SizedBox(height: 12),
              _menuEntry(13, _nexusConnectionActions(isDark)),
              const SizedBox(height: 20),
              _menuEntry(14, Center(child: Text('ESP32-C3 MINI • PD3.1 / QC3.0 • LOCAL CONTROL', style: TextStyle(color: AppColors.textFaint(isDark), fontSize: 8, letterSpacing: 1.4)))),
            ],
          ),
        )),
      ]),
    ...
    return _nexusCard(isDark, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SliderTheme(
        ...
        child: Slider(
          ...
          divisions: 20,
          label: '$fanSpeed%',
          onChangeStart: (v) => _fanSliderDragging = true,
          onChanged: (v) => setState(() => fanSpeed = v.round()),
          onChangeEnd: (v) {
            _fanSliderDragging = false;
            sendFanSpeed(v.round());
          },
        ),
      ),
  ...
  Widget _nexusRgbCard(bool isDark) {
    // Mode yang boleh diatur speed + brightness-nya. "custom" ikut disini
    // supaya slider KECEPATAN muncul kalau pola custom yang dipilih bergerak
    // (chaseRight/chaseLeft/pulse/spin/sparkle).
    const speedControlledModes = {'running', 'disco', 'bounce', 'knight', 'fire', 'chase', 'colorwave', 'custom'};
    final showSpeedSlider = speedControlledModes.contains(ledMode) && !(ledMode == 'custom' && customPattern == 'solid');
    // Warna custom sekarang berlaku utk mode built-in (knight/fire/chase/
    // colorwave/custom) DAN utk semua 100 mode di LED MENU (wledNNN).
    final showColorWheel = ledMode == 'knight' || ledMode == 'fire' || ledMode == 'chase' ||
        ledMode == 'colorwave' || ledMode == 'custom' || ledMode.startsWith('wled');
    final showWledSpeed = ledMode.startsWith('wled');
    final showCustomPatterns = ledMode == 'custom';
    final visibleLedModes = _visibleLedModeIds();

    return _nexusCard(isDark, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _nexusSectionTitle(isDark, 'RGB ENGINE', 'Efek LED dari firmware ESP32'),
      const SizedBox(height: 8),
      Row(children: [
        Icon(ledMode == "off" ? Icons.lightbulb_outline_rounded : Icons.lightbulb_rounded,
            color: ledMode == "off" ? AppColors.textFaint(isDark) : accentColor, size: 19),
        const SizedBox(width: 9),
        Text(ledMode == "off" ? 'LED OFF' : 'LED ON',
            style: TextStyle(
              color: ledMode == "off" ? AppColors.textFaint(isDark) : accentColor,
              fontSize: 11,
        ...
        Switch.adaptive(
          value: ledMode != "off",
          activeColor: accentColor,
          activeTrackColor: accentColor.withOpacity(.35),
          onChanged: _setLedPower,
        ),
      ]),
      if (visibleLedModes.isNotEmpty) ...[
        const SizedBox(height: 3),
        Text('MODE TERPILIH / FAVORIT',
            style: TextStyle(color: AppColors.textFaint(isDark), fontSize: 8, fontWeight: FontWeight.w900, letterSpacing: 1.3)),
        const SizedBox(height: 7),
        Wrap(
          spacing: 7,
          runSpacing: 7,
          children: visibleLedModes.map((mode) => _selectedLedModeChip(isDark, mode)).toList(),
        ),
      ],
      const SizedBox(height: 12),
      GestureDetector(
        behavior: HitTestBehavior.opaque,
        onTap: () => setState(() => ledMenuExpanded = !ledMenuExpanded),
        child: Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 9),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('LED MENU', style: TextStyle(color: accentColor, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1.3)),
              const SizedBox(height: 2),
              Text('100 mode RGB baru • ${favoriteLedModes.length} favorit',
                  style: TextStyle(color: AppColors.textFaint(isDark), fontSize: 8)),
            ])),
            Icon(ledMenuExpanded ? Icons.expand_less_rounded : Icons.expand_more_rounded,
      ...
      if (showCustomPatterns || showSpeedSlider || showWledSpeed || showColorWheel) ...[
        const SizedBox(height: 14),
        Divider(color: AppColors.textFaint(isDark).withOpacity(.15), height: 1),
        const SizedBox(height: 12),
        GestureDetector(
          behavior: HitTestBehavior.opaque,
          onTap: () => setState(() => ledCustomPanelExpanded = !ledCustomPanelExpanded),
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 11),
            decoration: BoxDecoration(
              color: accentColor.withOpacity(.07),
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 9),
              Expanded(child: Text('CUSTOM ${_ledModeDisplayName(ledMode).toUpperCase()}',
                  style: TextStyle(color: accentColor, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1.3))),
              Icon(ledCustomPanelExpanded ? Icons.expand_less_rounded : Icons.expand_more_rounded,
                  color: accentColor, size: 20),
            ]),
          ),
        ),
      ],
      if (ledCustomPanelExpanded) ...[
      if (showCustomPatterns) ...[
        const SizedBox(height: 14),
        Divider(color: AppColors.textFaint(isDark).withOpacity(.15), height: 1),
        const SizedBox(height: 12),
        Row(children: [
          Icon(Icons.motion_photos_on_rounded, color: accentColor, size: 16),
          const SizedBox(width: 8),
          Text('ANIMASI CUSTOM', style: TextStyle(color: AppColors.textFaint(isDark), fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1.4)),
        ]),
        const SizedBox(height: 10),
        Wrap(spacing: 7, runSpacing: 7, children: _customPatterns
            .map((p) => _customPatternChip(isDark, p.$2, p.$1, p.$3))
            .toList()),
      ],
      if (showSpeedSlider || showWledSpeed) ...[
        const SizedBox(height: 14),
        Divider(color: AppColors.textFaint(isDark).withOpacity(.15), height: 1),
        const SizedBox(height: 12),
        Row(children: [
        ...
        SliderTheme(
          ...
          child: Slider(
            ...
            divisions: 33,
            label: '$ledSpeed%',
            onChangeStart: (v) => _ledSpeedSliderDragging = true,
            onChanged: (v) => setState(() => ledSpeed = v.round()),
            onChangeEnd: (v) {
              _ledSpeedSliderDragging = false;
              sendLedSpeed(v.round());
            },
          ),
        ),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      ...
      if (showColorWheel) ...[
        ...
        Row(children: [
          ...
          Container(
            width: 20, height: 20,
            decoration: BoxDecoration(color: _activeEffectColor, shape: BoxShape.circle, border: Border.all(color: AppColors.textFaint(isDark).withOpacity(.3))),
          ),
          const SizedBox(width: 8),
          Text('#${_activeEffectColor.value.toRadixString(16).padLeft(8, '0').substring(2).toUpperCase()}',
            style: TextStyle(color: AppColors.text(isDark), fontSize: 11, fontWeight: FontWeight.w800)),
        ]),
        if (ledMode.startsWith('wled')) ...[
          const SizedBox(height: 6),
          Text('Warna ini cuma berlaku utk mode "${_ledModeDisplayName(ledMode)}" - mode lain punya warna masing-masing.',
              style: TextStyle(color: AppColors.textFaint(isDark), fontSize: 8.5)),
        ],
        const SizedBox(height: 14),
        Center(child: _ColorWheel(
          color: _activeEffectColor,
          size: 220,
          onChanged: (c) => setState(() {
            if (ledMode.startsWith('wled')) { wledColor = c; } else { customColor = c; }
          }),
          onChangeEnd: (c) => ledMode.startsWith('wled') ? sendWledColor(c) : sendCustomColor(c),
          onDragStart: () => setState(() => _colorWheelDragging = true),
          onDragEnd: () => setState(() => _colorWheelDragging = false),
        )),
        const SizedBox(height: 14),
        Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
          _rgbValueChip(isDark, 'R', _activeEffectColor.red, const Color(0xFFFF4444)),
          _rgbValueChip(isDark, 'G', _activeEffectColor.green, const Color(0xFF44FF66)),
          _rgbValueChip(isDark, 'B', _activeEffectColor.blue, const Color(0xFF4488FF)),
        ]),
      ],
      ],
      const SizedBox(height: 14),
      ...
      SliderTheme(
        ...
        child: Slider(
          ...
          divisions: 20,
          label: '$ledBrightness%',
          onChangeStart: (v) => _ledBrightnessSliderDragging = true,
          onChanged: (v) => setState(() => ledBrightness = v.round()),
          onChangeEnd: (v) {
            _ledBrightnessSliderDragging = false;
            sendLedBrightness(v.round());
          },
        ),
      ),
    ]));
  ...
  // Warna yang lagi ditampilkan di color wheel: warna khusus effect wled yang
  // aktif (tersimpan per-mode), atau customColor global utk mode built-in
  // (custom/knight/fire/chase/colorwave).
  Color get _activeEffectColor => ledMode.startsWith('wled') ? wledColor : customColor;

  List<String> _visibleLedModeIds() {
    // Urutan di sini HARUS stabil (gak boleh ikut geser cuma karena lagi
    // dipilih) - "Custom" selalu di posisi pertama, lalu mode yang di-star
    // tetap di urutan yang sama. Kalau mode yang lagi aktif belum termasuk
    // custom/favorit, baru ditambahkan di akhir (biar urutan yang sudah ada
    // gak ikut geser).
    final ids = <String>["custom"];
    for (final effect in _wledEffects) {
      final id = effect["id"];
      if (id != null && favoriteLedModes.contains(id)) ids.add(id);
    }
    if (ledMode != "off" && !ids.contains(ledMode)) ids.add(ledMode);
    return ids;
  }

  String _ledModeDisplayName(String mode) {
    const builtInNames = {
      "static": "Static",
      "running": "Running",
      "disco": "Disco",
    ...
    if (builtInNames.containsKey(mode)) return builtInNames[mode]!;
    for (final effect in _wledEffects) {
      if (effect["id"] == mode) return effect["name"]!;
    }
    return mode.toUpperCase();
  }

  Widget _selectedLedModeChip(bool isDark, String mode) {
    final selected = ledMode == mode;
    final favorite = favoriteLedModes.contains(mode);
    return GestureDetector(
      onTap: () => sendLed(mode),
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected ? accentColor.withOpacity(.16) : AppColors.card(isDark),
          borderRadius: BorderRadius.circular(11),
          border: Border.all(color: selected ? accentColor : accentColor.withOpacity(.14)),
        ),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(mode == "custom"
                  ? Icons.palette_rounded
                  : (selected ? Icons.play_circle_filled_rounded : Icons.graphic_eq_rounded),
              color: selected ? accentColor : AppColors.textFaint(isDark), size: 15),
          const SizedBox(width: 6),
          Text(_ledModeDisplayName(mode),
              style: TextStyle(color: selected ? accentColor : AppColors.text(isDark), fontSize: 9, fontWeight: FontWeight.w900)),
          if (favorite) ...[
            const SizedBox(width: 5),
            Icon(Icons.star_rounded, color: Colors.amberAccent, size: 13),
          ],
        ]),
      ),
    );
  ...
  Widget _wledMenu(bool isDark) {
    final favoriteEffects = _wledEffects.where((effect) => favoriteLedModes.contains(effect["id"])).toList();
    final otherEffects = _wledEffects.where((effect) => !favoriteLedModes.contains(effect["id"])).toList();
    final entries = <Map<String, String>>[
      ...favoriteEffects.map((effect) => {...effect, "section": "favorite"}),
      ...otherEffects.map((effect) => {...effect, "section": "all"}),
    ];
    ...
    return Container(
      ...
      child: ListView.separated(
        ...
        itemCount: entries.length + (favoriteEffects.isNotEmpty ? 1 : 0),
        separatorBuilder: (_, __) => Divider(height: 1, indent: 50, color: AppColors.textFaint(isDark).withOpacity(.08)),
        itemBuilder: (context, index) {
          if (favoriteEffects.isNotEmpty && index == 0) {
            return Padding(
              padding: const EdgeInsets.fromLTRB(15, 8, 15, 5),
              child: Text('FAVORIT', style: TextStyle(color: accentColor, fontSize: 8, fontWeight: FontWeight.w900, letterSpacing: 1.5)),
            );
          }
          final effectIndex = favoriteEffects.isNotEmpty ? index - 1 : index;
          final effect = entries[effectIndex];
          final id = effect["id"]!;
          final selected = ledMode == id;
          final isFavorite = favoriteLedModes.contains(id);
          return ListTile(
            dense: true,
            visualDensity: const VisualDensity(vertical: -2),
            leading: Icon(selected ? Icons.play_circle_filled_rounded : Icons.graphic_eq_rounded,
                color: selected ? accentColor : AppColors.textFaint(isDark), size: 18),
            title: Text(effect["name"]!, style: TextStyle(
                color: selected ? accentColor : AppColors.text(isDark),
                fontSize: 11, fontWeight: selected ? FontWeight.w900 : FontWeight.w600)),
            trailing: IconButton(
              tooltip: isFavorite ? 'Hapus dari favorit' : 'Simpan ke favorit',
              icon: Icon(isFavorite ? Icons.star_rounded : Icons.star_border_rounded,
                  color: isFavorite ? Colors.amberAccent : AppColors.textFaint(isDark), size: 21),
              onPressed: () => _toggleFavoriteLedMode(id),
            ),
            onTap: () {
              sendLed(id);
            },
  ...
  Widget _customPatternChip(bool isDark, String label, String pattern, IconData icon) {
    final selected = customPattern == pattern;
    return GestureDetector(
      onTap: () => sendCustomPattern(pattern),
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 180),
        padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 8),
        decoration: BoxDecoration(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(icon, color: selected ? accentColor : AppColors.textFaint(isDark), size: 15),
          const SizedBox(width: 6),
          Text(label, style: TextStyle(color: selected ? accentColor : AppColors.text(isDark), fontSize: 9, fontWeight: FontWeight.w900)),
        ]),
      ),
    );
  }
      ...
      if (hasCustomText) ...[
        ...
        SliderTheme(
          ...
          child: Slider(
            ...
            divisions: 33,
            label: '$oledTextSpeed%',
            onChangeStart: (v) => _oledTextSpeedSliderDragging = true,
            onChanged: (v) => setState(() => oledTextSpeed = v.round()),
            onChangeEnd: (v) {
              _oledTextSpeedSliderDragging = false;
              sendOledTextSpeed(v.round());
            },
          ),
        ),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
...
class _RunningMarqueeState extends State<_RunningMarquee> with SingleTickerProviderStateMixin {
  ...
  void _onTick(Duration elapsed) {
    ...
    // Normalisasi dengan modulo supaya tidak pernah membalik arah saat
    // melewati batas loop, termasuk setelah frame terlambat/drop.
    next %= loopWidth;
    ...
    return ClipRect(
      child: Transform.translate(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          widget.child,
          SizedBox(width: widget.gap),
          widget.child, // salinan ketiga menjaga layar tetap terisi saat loop
        ]),
      ),
    );
  }
...
// Entry menu masuk dari kiri secara bertahap. Satu controller dipakai bersama
// agar kartu-kartu utama terasa tersusun rapi, bukan muncul bersamaan.
class _StaggeredMenuEntry extends StatelessWidget {
  final Animation<double> controller;
  final int index;
  final Widget child;

  const _StaggeredMenuEntry({
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final start = (index * 0.065).clamp(0.0, 0.78).toDouble();
        final end = (start + 0.25).clamp(0.01, 1.0).toDouble();
